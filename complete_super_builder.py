...
def get_physics_pools():
    ...
    elec = [
        ...
        [make_q("PHY_ELEC_POT_DIVIDER", "Physics", "Electricity & Waves", "Medium", f"Potential divider: $R_1 = 100\\,\\Omega, R_2 = {100*i}\\,\\Omega, V_{{\\text{{in}}}} = 12\\text{{ V}}$. Find $V_{{\\text{{out}}}}$ across $R_2$.", f"${12.0 * i / (i + 1):.2f}\\text{{ V}}$", [f"${12.0 / (i + 1):.2f}\\text{{ V}}$", f"${12.0 * (i - 0.5) / (i + 1):.2f}\\text{{ V}}$", f"${12.0 * (i + 0.5) / (i + 1.5):.2f}\\text{{ V}}$", f"${6.0 / (i + 1):.2f}\\text{{ V}}$"], f"$V_{{\\text{{out}}}} = 12\\text{{ V}} \\times \\frac{{{100*i}}}{{100 + {100*i}}} = {12.0 * i / (i + 1):.2f}\\text{{ V}}$." ) for i in range(1, 7)],
        ...
        [make_q("PHY_ELEC_EMF_INTERNAL", "Physics", "Electricity & Waves", "Hard", f"Battery $\\mathcal{{E}} = 12\\text{{ V}}, r = {0.5*i}\\,\\Omega$, current $2\\text{{ A}}$. Terminal p.d. $V$?", f"${12.0 - 1.0*i:.1f}\\text{{ V}}$", [f"${12.0 - 0.5*i:.1f}\\text{{ V}}$", f"${12.0 - 2.0*i:.1f}\\text{{ V}}$", f"${12.0 + 1.0*i:.1f}\\text{{ V}}$", f"${6.0 - 0.5*i:.1f}\\text{{ V}}$"], f"$V = \\mathcal{{E}} - Ir = 12 - 2(0.5({i})) = {12.0 - 1.0*i:.1f}\\text{{ V}}$." ) for i in range(1, 7)],
        ...
        [make_q("PHY_ELEC_KIRCHHOFF2", "Physics", "Electricity & Waves", "Easy", f"In a closed loop with EMF ${6*i}\\text{{ V}}$ and resistors $R_1 = {i}\\,\\Omega, R_2 = {2*i}\\,\\Omega$, current $I$ is:", "$2.0\\text{ A}$", ["$1.0\\text{ A}$", "$0.5\\text{ A}$", "$3.0\\text{ A}$", "$4.0\\text{ A}$"], f"$I = \\frac{{{6*i}}}{{{3*i}}} = 2.0\\text{{ A}}$." ) for i in range(1, 7)],
